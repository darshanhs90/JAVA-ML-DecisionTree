...
public class aPruningVariance implements Serializable{
	/**
	...
	public   final aDecisionTreeVariance tree;
	public static double maxVal=0;
	public aDecisionTreeVariance tree2;
	public aPruningVariance(aDecisionTreeVariance aDecisionTreeVariance) throws Exception {
		tree=aDecisionTreeVariance;
	}

	public  aDecisionTreeVariance pruner(Integer l,Integer k) throws Exception{
		System.out.println("Pruning the tree now.Wait!!!!!!");
		aDecisionTreeVariance adBest = null;//=new aDecisionTreeVariance();
		adBest.b=false;
		adBest=new aDecisionTreeVariance(false);
		adBest.init();
		adBest=tree;
		aDecisionTreeVariance adDash=null;
		Random randM=new Random();
		Random randP=new Random();
		boolean rn=false;
		double accuracy=0;
		int m=0;
		aDecisionTreeVariance[] arrVal=new aDecisionTreeVariance[l+1];
		Node[] nodeVal=new Node[l+1];
		
		Double[] accrcy=new Double[l+1];
		arrVal[0]=(aDecisionTreeVariance) ObjectCloner.deepCopy(adBest);
		...
		for (int i = 1; i <= l; i++) {
			...
			adBest.init();
			if(adDash==null){
				 adDash=new aDecisionTreeVariance(false);
				adDash.init();
			...
			if(rn==false){
				if(adBest.calculateAccuracy2()>adDash.calculateAccuracy2()){
					...
					arrVal[i]=(aDecisionTreeVariance) ObjectCloner.deepCopy(adBest);
					...
					arrVal[i]=(aDecisionTreeVariance) ObjectCloner.deepCopy(adDash);
